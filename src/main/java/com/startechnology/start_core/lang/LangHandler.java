...
public class LangHandler {
    ...
    public static void init(RegistrateLangProvider provider) {
        ...
        provider.add("behaviour.start_core.bacteria.harvester_biomass_input",
                "§7Actual biomass consumption is §6100 * (2 ^ Metabolism)");
        provider.add("behaviour.start_core.bacteria.harvester_sugar_input",
                "§7Actual sugar consumption is §62 ^ Metabolism");
        provider.add("behaviour.start_core.bacteria.maximum_shown_output", "§7Maximum amount outputted shown");
        provider.add("behaviour.start_core.bacteria.harvester_primary_output",
                "§7Actual primary fluid output amount is §61500mB * Production");

        provider.add("behaviour.start_core.bacteria.secondary_output", "§5Secondary output");
        provider.add("behaviour.start_core.bacteria.harvester_secondary_output",
                "§7Actual secondary fluid output amount is §6750mB * Production");

        provider.add("behaviour.start_core.bacteria.tertiary_output", "§5Tertiary output");
        provider.add("behaviour.start_core.bacteria.harvester_tertiary_output",
                "§7Actual tertiary fluid output amount is §6250mB * Production");

        provider.add("behaviour.start_core.bacteria.super_output", "§5Super output");
        provider.add("behaviour.start_core.bacteria.harvester_super_output",
                "§7Actual super fluid output amount is §61000mB * Production");

        provider.add("behaviour.start_core.bacteria.bacteria_output", "§5Bacteria output");
        provider.add("behaviour.start_core.bacteria.harvester_bacteria_output",
                "§7Actual bacteria fluid output amount is §6500mB * Production");
        ...
        provider.add("behaviour.start_core.bacteria.vat_same_output",
                "§7Replicated bacteria with same stats & affinities as input");
        provider.add("behaviour.start_core.bacteria.vat_mutated_output",
                "§6Mutated §7bacteria with mutated stats but the same affinities as input");
        provider.add("behaviour.start_core.bacteria.mutator_affinity_output",
                "§6Mutated §7bacteria with random stats and affinities from input, but same type");
        provider.add("behaviour.start_core.bacteria.mutator_total_output",
                "§6Mutated §7bacteria with random stats, affinities and type.");
        ...
        provider.add("block.start_core.runic_mutator_description",
                "§7Using runic-age technologies, bacterial colonies are mutated through extreme genetic alterations to their biological structures, either reawakening dormant colonies or reshaping existing ones into novel types.");
        ...
        provider.add("block.start_core.rm1",
                "§7Reawaken dormant colonies into a randomised bacterial colony with random stats, affinities and type.");
        provider.add("block.start_core.gap", "");
        provider.add("block.start_core.rm3", "§bRunic Mutation:");
        provider.add("block.start_core.rm4",
                "§7Mutate bacterial colonies, changing their stats and affinities randomly along with their type depending on the type of runic plating used.");
        provider.add("block.start_core.harvester_description",
                "§7A precision bio-extraction device designed to process awakened bacterial colonies. It harnesses all outputs generated by the bacteria, feeding and accelerating their metabolic processes to maximize yield. The extraction process culminates in the bacteria's termination.");
        provider.add("block.start_core.hv0", "§eHydrocarbon Harvesting:");
        provider.add("block.start_core.hv1",
                "§7Extract primary, secondary, and tertiary outputs from bacterial colonies. §7The specific primary, secondary and tertiary outputs are determined by the §5Affinities §7of the bacteria.");
        provider.add("block.start_core.hv2",
                "§7The consumption of the bacteria scales with the §fMetabolism §7of the bacteria.");
        provider.add("block.start_core.hv3",
                "§7The amount fluid of output is base §fProduction§7 of the bacteria multiplied by their individual scalers.");
        provider.add("block.start_core.vat_description",
                "§7A specialized bioreactor designed for rapid bacterial replication and evolution. It processes a single bacterial sample, outputting duplicates while simultaneously producing mutated variants, fostering both consistency and diversity for experimental or industrial applications.");
        provider.add("block.start_core.vat1", "§eBacterial Breeding:");
        provider.add("block.start_core.vat2",
                "§7Replicate and mutate bacterial colonies, producing a large amount for usage in Harvesting or Mutation.");
        provider.add("block.start_core.vat3",
                "§7Produces §f16x Replicated§7 bacteria with the exact same type, stats and affinities.");
        provider.add("block.start_core.vat4",
        ...
        provider.add("start_core.machine.auxiliary_boosted_fusion_reactor.specific",
                "§7The reactor only takes {0}§7 energy hatches, Each hatch increases the buffer of the reactor by {1}M EU");
        provider.add("start_core.machine.fusion_reactor.overclocking",
                "§7Overclocks double energy and halve duration.");
        provider.add("start_core.machine.auxiliary_boosted_fusion_reactor.parallel_info",
                "§eAuxiliary Parallelization:");
        provider.add("start_core.machine.auxiliary_boosted_fusion_reactor.parallel_info_1",
                "§7Any Absolute Parallel Mastery Hatches can be used enabling high rates of production.");

        provider.add("gtceu.reflector_fusion_reactor", "Fusion Reactor");
        provider.add("start_core.multiblock.luv_fusion_reactor.description",
                "The Fusion Reactor MK I is a large multiblock structure used for fusing elements into heavier ones. It can only use §dLuV§r Energy Hatches. For every Hatch it has, its buffer increases by 10M EU, and has a maximum of 160M.");
        provider.add("start_core.multiblock.zpm_fusion_reactor.description",
                "The Fusion Reactor MK II is a large multiblock structure used for fusing elements into heavier ones. It can only use §cZPM§r Energy Hatches. For every Hatch it has, its buffer increases by 20M EU, and has a maximum of 320M.");
        provider.add("start_core.multiblock.uv_fusion_reactor.description",
                "The Fusion Reactor MK III is a large multiblock structure used for fusing elements into heavier ones. It can only use §3UV§r Energy Hatches. For every Hatch it has, its buffer increases by 40M EU, and has a maximum of 640M.");
        provider.add("start_core.multiblock.uev_fusion_reactor.description",
                "The Fusion Reactor MK IV is a large multiblock structure used for fusing elements into heavier ones. It can only use §aUEV§r Energy Hatches. For every Hatch it has, its buffer increases by 160M EU, and has a maximum of 2560M.");
        ...
        provider.add("start_core.machine.absolute_parallel_hatch_mk1.tooltip",
                "Allows to run up to 4 recipes in parallel.");
        provider.add("start_core.machine.absolute_parallel_hatch_mk2.tooltip",
                "Allows to run up to 8 recipes in parallel.");
        provider.add("start_core.machine.absolute_parallel_hatch_mk3.tooltip",
                "Allows to run up to 16 recipes in parallel.");
        provider.add("start_core.machine.absolute_parallel_hatch_energy.tooltip",
                "§6Without extra energy consumption per parallel.");

        provider.add("block.start_core.dream_link_energy_hatch_tooltip",
                "§7Wirelessly receiving energy through dreams");
        ...
        provider.add("start_core.machine.dream_link.tower.total_buffer_hover",
                "Total EU stored in the buffers of this tower");
        provider.add("start_core.machine.dream_link.hatch.input_per_sec_hover",
                "EU transferred to this hatch in the last second expressed in per tick");
        provider.add("start_core.machine.dream_link.tower.input_per_sec_hover",
                "EU being inputted to this tower in the last second expressed in per tick");
        provider.add("start_core.machine.dream_link.tower.output_per_sec_hover",
                "EU being transferred from this tower in the last second expressed in per tick");
        provider.add("start_core.machine.dream_link.tower.owner_hover",
                "EU will only be transmitted to hatches owned by this team/player.");
        ...
        provider.add("start_core.machine.dream_link.tower.range_hover",
                "The range of this tower in a square radius around (Any height allowed).");
        provider.add("start_core.machine.dream_link.tower.range_button_hover",
                "Display a render showing the range of the tower");
        ...
        provider.add("start_core.machine.dream_link.tower.connections_display_hover",
                "How many Dream-Link receivers are currently connected out of the maximum allowed.");
        ...
        provider.add("config.jade.plugin_start_core.dream_link_network_info",
                "[Star Technology] Dream-Link Network Info");
        provider.add("start_core.dream_link.cover.tooltip",
                "§7Supplies §f2§eA §f%d (%s§f) §7from the Dream-Network as a cover");
        ...
        provider.add("start_core.machine.dream_link_tower.beam_description",
                "§7The node focuses it's beam on a single device until it is full before continuing with the next. Allocation priority is determined based on the proximity of the device to the tower, with closer devices receiving energy first.");
        provider.add("start_core.machine.dream_link_tower.node_info", "§eNode Information:");
        provider.add("start_core.machine.dream_link_tower.range_description",
                "§7The node has a maximum square-radius of §d{0} §7blocks around it");
        provider.add("start_core.machine.dream_link_tower.connections_description",
                "§7The node has a maximum connections of §d{0}");
        provider.add("start_core.machine.dream_link_tower.infinite_connections_description",
                "§7The node has a §dinfinite§7 connections");
        provider.add("start_core.machine.dream_link_tower.copy_description",
                "§7The dream-network of this node can be copied using a §fLucinducer");
        provider.add("start_core.machine.dream_link_node.description",
                "§7Focuses a beam of energy through the dreamscape enabling lossless wireless transfer of energy to compatible dream-link devices.");
        provider.add("start_core.machine.oneiric_relay.description",
                "§7Refines and amplifies dream-energy pathways, enabling a greater distance for beams to travel through the dreamscape.");
        provider.add("start_core.machine.daydream_spire.description",
                "§7Manipulates semi-lucid thought currents for extended dreamscape energy transmission.");
        provider.add("start_core.machine.beacon_of_lucidity.description",
                "§7Channels pure lucid clarity to maintain dream-link integrity across the entire dreamscape of a dimension.");
        provider.add("start_core.machine.paragon_of_the_veil.description",
                "§7The Paragon integrates directly with the substrate of the dream realm itself. Rather than projecting energy, it becomes a universal synchronization anchor, allowing instantaneous energy transmission to all connected dream-link devices, regardless of dimension.");
        provider.add("start_core.machine.dream_link_tower.beacon_of_lucidity.range_description",
                "§7The beacon has §dunlimited range §7in the same dimension as it");
        provider.add("start_core.machine.dream_link_tower.paragon_of_the_veil.range_description",
                "§7The Paragon has §dunlimited cross-dimensional range");
        ...
        provider.add("behaviour.start_core.hellforge.max_heat",
                "§cMaximum temperature of this heating fluid is §l%sMK");
        ...
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_description",
                "§7An exalted crucible that harnesses the Soul of the Flame to fuel its hellish fury, demanding fluid offerings to further awaken its blazing fury.");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d0", "§9Hadean Crucible:");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d1",
                "§7The engine utilises the crucible to alloy materials together at extreme temperatures, but the Soul of the Flame can only heat the crucible to §b925MK§7 itself.");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d2",
                "§7Provide different §dHeating Fluids §7to the engine to heat up the crucible beyond the soul's limit. Each fluid has a maximum temperature that it can heat the crucible to.");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d9",
                "§7These heating recipes will always be prioritised over the other recipes during recipe search.");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d3",
                "§7The engine will always lose §93MK §7of its heat every 10 seconds. When dormant the engine loses an additional §972MK§7 every 10 seconds.");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d5", "§5Infernal Parallelization:");
        provider.add("block.start_core.fornaxs_infernal_rotary_engine_d6",
                "§7For every n §d450MK§7 over the recipe requirement, the engine gains 2^n absolute parallels.");
        ...
        provider.add("block.start_core.hellforge_description",
                "§7A towering infernal crucible that harnesses the Heart of the Flame to fuel its relentless fires, demanding fluid offerings to awaken its molten fury.");
        provider.add("block.start_core.hellforge_d0", "§6Hadean Crucible:");
        provider.add("block.start_core.hellforge_d1",
                "§7The hell forge utilises the crucible to alloy materials together at an extreme temperature, but the Heart of the Flame cannot heat the crucible itself.");
        provider.add("block.start_core.hellforge_d2",
                "§7Provide different §cHeating Fluids §7to the hell forge to heat up the crucible. Each fluid has a maximum temperature that it can heat the crucible to.");
        provider.add("block.start_core.hellforge_d9",
                "§7These heating recipes will always be prioritised over the other recipes during recipe search.");
        provider.add("block.start_core.hellforge_d3",
                "§7The hell forge will always lose §95MK §7of its heat every 10 seconds. When dormant the hell forge loses an additional §9120MK§7 every 10 seconds.");
        provider.add("block.start_core.hellforge_d5", "§eInfernal Parallelization:");
        provider.add("block.start_core.hellforge_d6",
                "§7For every n §c450MK§7 over the recipe requirement, the hell forge gains 2^n absolute parallels.");
        provider.add("machine.start_core.redstone_interfacing", "§cRedstone Variadic Interfacing:");
        provider.add("block.start_core.hellforge_d8",
                "§7Provides §fPercentage to nMK §7indicators for each heating fluid temperature cap.");
        ...
        provider.add("config.jade.plugin_start_core.abyssal_harvester_info",
                "[Star Technology] Abyssal Harvester Entropy");

        provider.add("start_core.redstone_hatch.d0",
                "Provides access to redstone indicators for multiblock automation");
        ...
        provider.add("config.jade.plugin_start_core.variadic_redstone_info",
                "[Star Technology] Variadic Redstone Information");
        ...
        provider.add("start_core.machine.abyssal_harvester.description",
                "§7Absorbs the latent tides of the end abyss, the harvester draws forth exotic abyssal fluids from dimensional fractures and concentrates it as a material fluid.");
        provider.add("start_core.machine.abyssal_harvester.ah0", "§6Abyssal Saturation:");
        provider.add("start_core.machine.abyssal_harvester.ah1",
                "§7The abyssal harvester will continually become more and more saturated with the latent abyss allowing for different blends of outcomes.");
        provider.add("start_core.machine.abyssal_harvester.ah2",
                "§7The machine will always gain abyssal saturation at §f0.55% per 5s§7 up to a maximum of §f120.00%§7.");
        provider.add("start_core.machine.abyssal_harvester.ah3",
                "§7Running recipes in the harvester will decrease the current saturation by §f5.00%§7.");
        provider.add("start_core.machine.abyssal_harvester.ah4", "§6Abyssal Stabilization:");
        provider.add("start_core.machine.abyssal_harvester.ah5",
                "§7When the Saturation is within a §f+5.00% or -5.00%§7 range of §f22.50%, 52.50%, or 82.50%§7 it gets a free 2x parallel.");
        provider.add("start_core.machine.abyssal_harvester.ah6",
                "§7Provides §fPercentage to n saturation percent §7indicators for certain n.");
        ...
        provider.add("config.jade.plugin_start_core.vacuum_chemical_reaction_chamber_info",
                "[Star Technology] Vacuum Chemical Reaction Chamber Info");
        ...
        provider.add("start_core.multiblock.nyinsane_turbine.bec_og_boosting",
                "Oganesson Stabilised Bose-Einstein Condensate Boosted");
        ...
        provider.add("start_core.abyssal_containment_room.acr0",
                "§7You think you peer into its silence... but silence is the tongue of eternity, and §keternity§r§7 has already spoken your name.");
        provider.add("start_core.abyssal_containment_room.acr1", "§dAbyssal Containment");
        provider.add("start_core.abyssal_containment_room.acr2",
        ...
        provider.add("item.start_core.lucinducer.tooltip2",
                "§7While held offhand, the stored network will be applied to any placed Dream-Link Hatch or Cover.");
        provider.add("item.start_core.lucinducer.tooltip3",
                "§7When equipped as a Curio, it charges EU and FE-powered items in your inventory by using the copied Dream-Link network.");

        provider.add("curios.identifier.lucinducer", "Lucinducer");

        provider.add("item.start_core.mechanical_memory_card.tooltip",
                "§7Allows you to copy miscellaneous machines information. Supported: ");
        ...
        provider.add("start_core.mechanical_memory_card.paste_settings",
                "Applied settings from Mechanical Memory Card");

        provider.add("block.start_core.threading_controller.tooltip",
                "Allows you to access and control the threading system by assigning points to various stats.\n§6The available points depend on the helixes you have installed in the multiblock structure.");
        provider.add("start_core.machine.threading_controller.list_components", "§6Threading Components:");
        provider.add("start_core.machine.threading_controller.list_components.hover",
                "The current amount of each threading component block in this machine");
        ...
        provider.add("start_core.machine.threading_controller.stat.general_hover",
                "These points can be assigned to any others");
        provider.add("start_core.machine.threading_controller.stat.display_assign", "%s %s §7Assigned %s §7Total");
        provider.add("start_core.machine.threading_controller.stat.display", "§6Specialization:");
        provider.add("start_core.machine.threading_controller.stat.display.hover",
                "View different points and their current assignments, along with assigning points from Generalis to others");
        ...
        provider.add("start_core.machine.threading_controller.stat.speed.hover",
                "0.5^a Modifier per 100 * sum(a) Points. (1 - 0.5^a) * 100 Percent Duration Reduction. Where 'a' is the desired times duration halved");
        provider.add("start_core.machine.threading_controller.stat.efficiency.hover",
                "(1 - 30/(30 + Efficienta)) * 100 Percent Total Power Reduction");
        provider.add("start_core.machine.threading_controller.stat.parallels.hover",
                "1 Parallel per 20 Points. Increases duration by Parallel^0.5");
        provider.add("start_core.machine.threading_controller.stat.threading.hover",
                "1 Thread per 5 Points (Each thread processes a different recipe type)");
        provider.add("start_core.machine.threading_controller.stat.assign", "§e[+]");
        provider.add("start_core.machine.threading_controller.stat.assign.hover",
                "Assign 1 Generalis to this (SHIFT for 5); (CTRL for 20); (SHIFT + CTRL for max)");
        provider.add("start_core.machine.threading_controller.stat.remove", "§e[-]");
        provider.add("start_core.machine.threading_controller.stat.remove.hover",
                "Unassign 1 Generalis from this (SHIFT for 5); (CTRL for 20); (SHIFT + CTRL for max)");
        ...
        provider.add("block.start_core.t1_combustion_module_d2",
                "Supply §f80 mB/s§7 of §bWhite Fuming Nitric Acid §7to produce up to §f%s EU/t§7 at §f2x§7 fuel consumption.");
        ...
        provider.add("block.start_core.t2_combustion_module_d2",
                "Supply §f120 mB/s§7 of §bRed Fuming Nitric Acid §7to produce up to §f%s EU/t§7 at §f2x§7 fuel consumption.");
        ...
        provider.add("block.start_core.t1_rocket_module_d1",
                "Uses §f%d B/hr §7of §6Tungsten Disulfide §7while working");
        provider.add("block.start_core.t1_rocket_module_d2",
                "Supply §f210 mB/s§7 of §bDioxygen Difluoride §7to produce up to §f%s EU/t§7 at §f2x§7 fuel consumption.");
        ...
        provider.add("block.start_core.t2_rocket_module_d1",
                "Uses §f%d B/hr §7of §6Tungsten Disulfide §7while working");
        provider.add("block.start_core.t2_rocket_module_d2",
                "Supply §f240 mB/s§7 of §bFerrocenium Superoxide §7to produce up to §f%s EU/t§7 at §f2x§7 fuel consumption.");
        ...
        provider.add("block.start_core.modular_combustion_frame_d1",
                "§7A Modular Frame that can hold up to 8 different Combustion or Rocket Modules");
        provider.add("block.start_core.modular_combustion_frame_d2",
                "§3Cooling Boosting: §7Consumes 500B an hour per module connected of §fDistilled Water§7 for a §a+20% §fEU/t §7output, or with §fDe-Ionized Water§7 for a §a+40% §fEU/t §7output, and a providing no coolant leads to a §4-10% §fEU/t §7to the energy output");
        provider.add("block.start_core.modular_combustion_frame_d3",
                "§cModular Frame: §7Place the Module so its §fTerminal Hatch §7faces directly into a §fNode Hatch §7on the Multiblock. The §fTerminal Hatch §7UI will show whether the connection is active and which Multiblock this Module connects to. The §fFrame §7enables all §fModule Machines §7attached to it.");
        ...
        provider.add("modular.start_core.not_formed_description",
                "The multiblock must be formed for a link to be established.");
        provider.add("modular.start_core.supported_list_description",
                "These are the available multiblock types which can be linked with this terminal to enable their functionality.");
        provider.add("config.jade.plugin_start_core.modular_interface_hatch_part_machine",
                "[Star Technology] Modular Interface Information");
        ...
        provider.add("variadic.start_core.description.default",
                "When part of a formed multiblock which has interfaced with this hatch, clicking here will produce a dropdown to select a desired redstone indicator of the multiblock's function");
        provider.add("variadic.start_core.indicator.hellforge", "Percentage to §c%s");
        provider.add("variadic.start_core.description.hellforge",
                "A clamped value between 0-15 of the (current temperature / %sMK) * 15");
        provider.add("variadic.start_core.indicator.abyssal_harvester", "Ratio of §5%s §rAbyssal Saturation");
        provider.add("variadic.start_core.description.abyssal_harvester",
                "A clamped value between 0-15 of the (current saturation / %s%%) * 15");

        provider.add("block.start_core.komaru.tooltip1", "§6§lModular Gateway of the Heavens");

        provider.add("block.start_core.komaru.paginated1.1",
                "§7To access the worlds beyond this reality the draconic empires utilized all understanding of technology. From that, Dr.Conic and the surrounding visionaries were able to construct a gateways to heavens beyond their understanding. By honing §dFaetic§7 and §5Draconic§7 powers they were able to find fractal like spatial realms which largely stood outside of time. They harvested them to fuel their own growth... and their greed had a grave cost. For the energies utilized to break into realms beyond, are the same ones that can shatter a world completely and leave it to be consumed by the abyss. Forever marking a world with a reminder of what happens when trying to control powers barely understood.");
        provider.add("block.start_core.komaru.paginated1.2",
                "§7If one is to understand and stabilize these powers then reward, not disaster awaits. Harvesting what the Draconic Empires could never is a blessing and a curse just the same. §kkomaru cat§7 awaits what you decide next.");

        provider.add("block.start_core.komaru.paginated2.1", "§bModular Overclocking");
        provider.add("block.start_core.komaru.paginated2.2",
                "§7Each §6K.O.M.A.R.U§7 has the ability to store 4 Advanced Modules and 20 Basic Modules. Each Basic Module is overclocked to 1/64th of the amperage input up to §f64§eA§7. Each Advanced Module is overclocked to 1/16th of the amperage input up to §f256§eA§7. To achieve this §6K.O.M.A.R.U§7 accepts §dLaser Hatches§7.");
        provider.add("block.start_core.komaru.paginated2.3", "§d§lFaetic Scaling");
        provider.add("block.start_core.komaru.paginated2.4",
                "§7Compromised of the product of §dFaetic Scaling§7 and §9Modular Efficiency§7 where:");
        provider.add("block.start_core.komaru.paginated2.5",
                "    §dFaetic Scaling§7 = 1.025 ^ ([Number of Basic Modules] + 3 * [Number of Advanced Modules])");
        provider.add("block.start_core.komaru.paginated2.6",
                "    §9Modular Efficiency§7 = 0.99 ^ ([Number of Basic Modules]) * 0.98 ^ ([Number of Advanced Modules])");

        provider.add("block.start_core.komaru.paginated3.1", "§3§lFilament Stabilization");
        provider.add("block.start_core.komaru.paginated3.2",
                "§7By using higher tiers of filaments you can more effectively stabilize the riftic connection but the frame will consume all available filaments it can per cycle (10s).");
        provider.add("block.start_core.komaru.paginated3.3",
                "    §3Filament Stabilization§7 = (1 - 0.025 * [Tier of Filament]) ^ ([Filaments Available] / 5)");
        provider.add("block.start_core.komaru.paginated3.4", "§f§lFaematter Consumption");
        provider.add("block.start_core.komaru.paginated3.5",
                "§7To enable the modules to run and the riftic connection the frame requires a constant supply of Faematter to run that if unchecked will increase exponentially.");
        provider.add("block.start_core.komaru.paginated3.6",
                "§7The amount of Faematter consumed floors at 10B per cycle and ceilings at 50B per cycle.");
        provider.add("block.start_core.komaru.paginated3.7",
                "    §fFaematter Consumption§7 = [Consumption of Faematter in Prior Cycle] * §3Filament Stabilization§7 * §dFaetic Scaling");
        provider.add("block.start_core.komaru.paginated3.8",
                "    §7If this is the first cycle the minimum of 10B will be assumed as the prior cycle consumption");

        provider.add("block.start_core.komaru.paginated4.1", "§4§lDire Consequences");
        provider.add("block.start_core.komaru.paginated4.2",
                "§7The §6K.O.M.A.R.U§7 should not be unchecked as it scales the power it demands by the faematter it consumes.");
        provider.add("block.start_core.komaru.paginated4.3",
                "- §7If stabilized you will be awarded power §aefficiency§7.");
        provider.add("block.start_core.komaru.paginated4.4",
                "- §7If left unchecked you will face §4dire consequences§7.");
        ...
        provider.add("variadic.start_core.description.komaru.c",
                "A clamped value between 0-15 of (current C / %s) * 15");
        provider.add("variadic.start_core.indicator.komaru.fae_scaling", "Ratio of §dFae Scaling §rto §e%s");
        provider.add("variadic.start_core.description.komaru.fae_scaling",
                "A clamped value between 0-15 of (current Fae Scaling / %s) * 15");
        ...
        provider.add("solar.start_core.solar_cell.tooltip3",
                "§7Produces power only when used in a Solar Panel or Array.");
        ...
        provider.add("block.start_core.solar_machine.tooltip1",
                "§7Use §fSolar Cells§7 to harvest the power of the §eSUN§7. This machine will combine the produced power of all connected solar cells and replace broken ones when they decay if you supply it with new ones.");
        provider.add("block.start_core.solar_machine.tooltip2",
                "§fSolar Cells§7 will only produce power if they can see the sun.");

        provider.add("block.start_core.solar_machine.paginated1.1", "§a§lVoltage multiplier:§f x%s");
        provider.add("block.start_core.solar_machine.paginated1.2",
                "§7Total EU/t generation is increased by this factor.");

        provider.add("block.start_core.solar_machine.paginated1.3", "§c§lSolar Cell Decay");
        provider.add("block.start_core.solar_machine.paginated1.4",
                "§7Solar cells complete a cycle every 6 seconds, after which they will lose a part of their durability, depending on their heat.");

        provider.add("block.start_core.solar_machine.paginated2.1", "§4§lOverheating");
        provider.add("block.start_core.solar_machine.paginated2.2",
                "§7Each Solar Cell has its own max temperature, and have a minimum temperature of §f273K§7.");
        provider.add("block.start_core.solar_machine.paginated2.3",
                "§4Overheat§f%§7 = §f[Current Temperature - 273]§7 / §f[Max Temperature - 273]");
        provider.add("block.start_core.solar_machine.paginated2.4",
                "§7Based on the §4overheat§f%§7 durability loss per cycle is increased, with the following multipliers:");
        ...
        provider.add("block.start_core.solar_panel.paginated1.2",
                "§7Each cell will increase its temperature by §f0.2K§7 per 6 second cycle while producing power.");
        provider.add("block.start_core.solar_panel.paginated1.3",
                "§7Each cell will decrease its temperature by §f0.1K§7 per 6 second cycle while inactive.");

        provider.add("block.start_core.solar_array.paginated1.1", "§e§lSolar Cell Thermodynamics");
        provider.add("block.start_core.solar_array.paginated1.2",
                "§7Each cell will increase its temperature by §f0.3K§7 per 6 second cycle while producing power.");
        provider.add("block.start_core.solar_array.paginated1.3",
                "§7Each cell will decrease its temperature by §f0.1K§7 per 6 second cycle while inactive.");

        provider.add("block.start_core.solar_array.paginated2.1", "§3§lSolar Array Cooling");
        provider.add("block.start_core.solar_array.paginated2.2",
                "§7By providing §f%s§7 buckets of §f%s§7 every hour the array will become cooled.");
        provider.add("block.start_core.solar_array.paginated2.3",
                "§7While cooled solar cell thermodynamics will be adjusted.");
        provider.add("block.start_core.solar_array.paginated2.4",
                "§7Each cell will increase its temperature by §f0.18K§7 per 6 second cycle while producing power.");
        provider.add("block.start_core.solar_array.paginated2.5",
                "§7While cooled the solar array will increase its voltage multiplier to §fx%s");
        ...
        provider.add("gtceu.parallel_type.bulk_processing", "- %dx from Bulk Processing");
